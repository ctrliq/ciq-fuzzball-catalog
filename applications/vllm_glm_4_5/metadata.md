...
Serves [`zai-org/GLM-4.5`](https://recipes.vllm.ai/zai-org/GLM-4.5) from an
...
[vLLM recipe for GLM-4.5](https://recipes.vllm.ai/zai-org/GLM-4.5): every vLLM
...
GLM-4.5 MoE language model (~358B total parameters, BF16) with built-in MTP
layers for speculative decoding and native tool calling

## Model

- **Checkpoint**: `zai-org/GLM-4.5`
...
| --- | --- | --- | --- |
| `default` | bf16 | 859 GB | `zai-org/GLM-4.5` |
| `fp8` | fp8 | 430 GB | `zai-org/GLM-4.5-FP8` |
...
```sh
fuzzball workflow catalog start vllm_glm_4_5
fuzzball workflow catalog start vllm_glm_4_5 --values Hardware=mi355x,Strategy=single_node_tp,Variant=fp8
fuzzball workflow catalog start vllm_glm_4_5 --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `zai-org/GLM-4.5` regardless of the variant
...
vllm serve zai-org/GLM-4.5 \
  ...
  --served-model-name zai-org/GLM-4.5
```
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/zai-org/GLM-4.5. Regenerate this application with
