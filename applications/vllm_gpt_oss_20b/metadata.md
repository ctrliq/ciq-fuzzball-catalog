...
Serves [`openai/gpt-oss-20b`](https://recipes.vllm.ai/openai/gpt-oss-20b) from
...
from the [vLLM recipe for GPT-OSS
20B](https://recipes.vllm.ai/openai/gpt-oss-20b): every vLLM flag, environment
variable, container image, GPU count and parallel layout below is what that
recipe validated for the selected hardware.

OpenAI's gpt-oss-20b — 21B-total / 3.6B-active MoE reasoning model with native
MXFP4 quant; supports GPU, XPU, and Xeon 6 CPU serving
...
-  **`EnableToolCalling`** (default on): OpenAI harmony tool-call parser with
  automatic tool choice

## Usage

```sh
fuzzball workflow catalog start vllm_gpt_oss_20b
fuzzball workflow catalog start vllm_gpt_oss_20b --values Hardware=mi355x
fuzzball workflow catalog start vllm_gpt_oss_20b --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `openai/gpt-oss-20b` regardless of the variant
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/openai/gpt-oss-20b. Regenerate this application with
