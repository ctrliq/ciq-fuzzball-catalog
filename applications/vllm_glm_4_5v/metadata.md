...
---
id: "ciq/ml_and_ai/vllm_glm_4_5v"
name: "vLLM GLM-4.5V"
category: "ML_AND_AI"
tags:
- LLM
...
Serves [`zai-org/GLM-4.5V`](https://recipes.vllm.ai/zai-org/GLM-4.5V) from an
...
[vLLM recipe for GLM-4.5V](https://recipes.vllm.ai/zai-org/GLM-4.5V): every
...
GLM-4.5 vision-language MoE model (~107B parameters, BF16) with
image-text-to-text capability, 64K context, expert parallelism, and native FP8
...
| --- | --- | --- | --- |
| `default` | bf16 | 257 GB | `zai-org/GLM-4.5V` |
| `fp8` | fp8 | 128 GB | `zai-org/GLM-4.5V-FP8` |
...
```sh
fuzzball workflow catalog start vllm_glm_4_5v
fuzzball workflow catalog start vllm_glm_4_5v --values Hardware=mi355x,Strategy=single_node_tp,Variant=fp8
fuzzball workflow catalog start vllm_glm_4_5v --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `zai-org/GLM-4.5V` regardless of the variant
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/zai-org/GLM-4.5V. Regenerate this application with
