...
---
id: "ciq/ml_and_ai/vllm_nvidia_nemotron_3_nano_4b_bf16"
name: "vLLM NVIDIA-Nemotron-3-Nano-4B-BF16"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves
[`nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16`](https://recipes.vllm.ai/nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16)
...
Nemotron-3-Nano-4B](https://recipes.vllm.ai/nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16):
...
NVIDIA Nemotron-3-Nano 4B (Mamba-hybrid dense) — compact reasoning + tool-use
model with BF16 and FP8 variants
...
| --- | --- | --- | --- |
| `default` | bf16 | 10 GB | `nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16` |
| `fp8` | fp8 | 5 GB | `nvidia/NVIDIA-Nemotron-3-Nano-4B-FP8` |

## Features

-  **`EnableToolCalling`** (default on): Qwen3 Coder tool-call parser with
  automatic tool choice
-  **`EnableReasoning`** (default on): Custom Nano v3 reasoning parser
  (download plugin: nano_v3_reasoning_parser.py)

## Usage

```sh
fuzzball workflow catalog start vllm_nvidia_nemotron_3_nano_4b_bf16
fuzzball workflow catalog start vllm_nvidia_nemotron_3_nano_4b_bf16 --values Hardware=mi355x,Variant=fp8
fuzzball workflow catalog start vllm_nvidia_nemotron_3_nano_4b_bf16 --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16` regardless
...
vllm serve nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16 \
  ...
  --served-model-name nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16
```

Image: `docker://vllm/vllm-openai:v0.11.2`
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16. Regenerate
