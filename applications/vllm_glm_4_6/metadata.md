...
Serves [`zai-org/GLM-4.6`](https://recipes.vllm.ai/zai-org/GLM-4.6) from an
...
[vLLM recipe for GLM-4.6](https://recipes.vllm.ai/zai-org/GLM-4.6): every vLLM
...
GLM-4.6 MoE language model (~357B total parameters, BF16) with MTP speculative
decoding, native tool calling and reasoning

## Model

- **Checkpoint**: `zai-org/GLM-4.6`
- **Architecture**: moe, 357B parameters
...
| --- | --- | --- | --- |
| `default` | bf16 | 857 GB | `zai-org/GLM-4.6` |
| `fp8` | fp8 | 428 GB | `zai-org/GLM-4.6-FP8` |
...
```sh
fuzzball workflow catalog start vllm_glm_4_6
fuzzball workflow catalog start vllm_glm_4_6 --values Hardware=mi355x,Strategy=single_node_tp,Variant=fp8
fuzzball workflow catalog start vllm_glm_4_6 --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `zai-org/GLM-4.6` regardless of the variant
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/zai-org/GLM-4.6. Regenerate this application with
