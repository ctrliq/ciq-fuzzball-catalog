...
---
id: "ciq/ml_and_ai/vllm_qwen3_4b"
name: "vLLM Qwen3-4B"
category: "ML_AND_AI"
tags:
- LLM
...
Serves [`Qwen/Qwen3-4B`](https://recipes.vllm.ai/Qwen/Qwen3-4B) from an
...
[vLLM recipe for Qwen3-4B](https://recipes.vllm.ai/Qwen/Qwen3-4B): every vLLM
...
Qwen3 4B dense model with hybrid thinking/non-thinking modes — fits on a single
TPU v6e chip, one GPU or one Xeon 6 NUMA node.
...
| --- | --- | --- | --- |
| `default` | bf16 | 10 GB | `Qwen/Qwen3-4B` |
| `fp8` | fp8 | 5 GB | `Qwen/Qwen3-4B-FP8` |
...
```sh
fuzzball workflow catalog start vllm_qwen3_4b
fuzzball workflow catalog start vllm_qwen3_4b --values Hardware=mi355x,Variant=fp8
fuzzball workflow catalog start vllm_qwen3_4b --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `Qwen/Qwen3-4B` regardless of the variant served,
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/Qwen/Qwen3-4B. Regenerate this application with
