...
---
id: "ciq/ml_and_ai/vllm_kimi_k2_5"
name: "vLLM Kimi-K2.5"
category: "ML_AND_AI"
tags:
- LLM
...
Serves [`moonshotai/Kimi-K2.5`](https://recipes.vllm.ai/moonshotai/Kimi-K2.5)
...
Kimi-K2.5](https://recipes.vllm.ai/moonshotai/Kimi-K2.5): every vLLM flag,
...
| --- | --- | --- | --- |
| `default` | int4 | 714 GB | `moonshotai/Kimi-K2.5` |
| `mxfp4` | mxfp4 | 669 GB | `amd/Kimi-K2.5-MXFP4` |
| `nvfp4` | nvfp4 | 600 GB | `nvidia/Kimi-K2.5-NVFP4` |
...
-  **`EnableSpecDecoding`** (default on): Eagle3 speculative decoding for
  accelerated inference (requires vLLM >= 0.18.0)
...
```sh
fuzzball workflow catalog start vllm_kimi_k2_5
fuzzball workflow catalog start vllm_kimi_k2_5 --values Hardware=mi355x,Strategy=single_node_tp,Variant=nvfp4
fuzzball workflow catalog start vllm_kimi_k2_5 --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `moonshotai/Kimi-K2.5` regardless of the variant
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/moonshotai/Kimi-K2.5. Regenerate this application
