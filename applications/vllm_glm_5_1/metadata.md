...
Serves [`zai-org/GLM-5.1`](https://recipes.vllm.ai/zai-org/GLM-5.1) from an
...
[vLLM recipe for GLM-5.1](https://recipes.vllm.ai/zai-org/GLM-5.1): every vLLM
...
GLM-5.1 refreshed version of GLM-5 — frontier-scale MoE language model (~744B
total parameters) with MTP speculative decoding and thinking mode
...
| --- | --- | --- | --- |
| `default` | bf16 | 1786 GB | `zai-org/GLM-5.1` |
| `fp8` | fp8 | 893 GB | `zai-org/GLM-5.1-FP8` |
| `nvfp4` | nvfp4 | 446 GB | `nvidia/GLM-5.1-NVFP4` |
...
```sh
fuzzball workflow catalog start vllm_glm_5_1
fuzzball workflow catalog start vllm_glm_5_1 --values Hardware=mi355x,Strategy=single_node_tp,Variant=nvfp4
fuzzball workflow catalog start vllm_glm_5_1 --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `zai-org/GLM-5.1` regardless of the variant
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/zai-org/GLM-5.1. Regenerate this application with
