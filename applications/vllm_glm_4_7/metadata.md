...
Serves [`zai-org/GLM-4.7`](https://recipes.vllm.ai/zai-org/GLM-4.7) from an
...
[vLLM recipe for GLM-4.7](https://recipes.vllm.ai/zai-org/GLM-4.7): every vLLM
...
GLM-4.7 MoE language model (~358B total parameters) with MTP speculative
decoding, updated tool call parser, and reasoning support

## Model

- **Checkpoint**: `zai-org/GLM-4.7`
- **Architecture**: moe, 358B parameters
...
| --- | --- | --- | --- |
| `default` | bf16 | 859 GB | `zai-org/GLM-4.7` |
| `fp8` | fp8 | 430 GB | `zai-org/GLM-4.7-FP8` |
| `nvfp4` | nvfp4 | 215 GB | `nvidia/GLM-4.7-NVFP4` |
...
-  **`EnableReasoning`** (default on): GLM-4.5 reasoning parser for
  chain-of-thought extraction
-  **`EnableSpecDecoding`** (default off): Multi-Token Prediction speculative
  decoding using the model's built-in MTP layers

## Usage

```sh
fuzzball workflow catalog start vllm_glm_4_7
fuzzball workflow catalog start vllm_glm_4_7 --values Hardware=mi355x,Strategy=single_node_tp,Variant=nvfp4
fuzzball workflow catalog start vllm_glm_4_7 --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `zai-org/GLM-4.7` regardless of the variant
...
-  The recipe lists extra install steps that apply to its pip install path; the
  ...
  -  `uv pip install -U vllm --pre --extra-index-url
    https://wheels.vllm.ai/nightly`
  -  `uv pip install git+https://github.com/huggingface/transformers.git`
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/zai-org/GLM-4.7. Regenerate this application with
