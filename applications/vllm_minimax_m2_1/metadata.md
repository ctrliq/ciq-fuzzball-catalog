...
---
id: "ciq/ml_and_ai/vllm_minimax_m2_1"
name: "vLLM MiniMax-M2.1"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves
[`MiniMaxAI/MiniMax-M2.1`](https://recipes.vllm.ai/MiniMaxAI/MiniMax-M2.1) from
...
MiniMax-M2.1](https://recipes.vllm.ai/MiniMaxAI/MiniMax-M2.1): every vLLM flag,
...
MiniMax M2.1 MoE language model (230B total / 10B active) for coding, agent
toolchains, and long-context reasoning — native FP8 checkpoint

## Model

- **Checkpoint**: `MiniMaxAI/MiniMax-M2.1`
...
```sh
fuzzball workflow catalog start vllm_minimax_m2_1
fuzzball workflow catalog start vllm_minimax_m2_1 --values Hardware=mi355x,Strategy=single_node_tp
fuzzball workflow catalog start vllm_minimax_m2_1 --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `MiniMaxAI/MiniMax-M2.1` regardless of the variant
...
vllm serve MiniMaxAI/MiniMax-M2.1 \
  ...
  --served-model-name MiniMaxAI/MiniMax-M2.1
```
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/MiniMaxAI/MiniMax-M2.1. Regenerate this application
