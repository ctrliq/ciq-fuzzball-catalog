...
---
id: "ciq/ml_and_ai/vllm_qwen3_32b"
name: "vLLM Qwen3-32B"
category: "ML_AND_AI"
tags:
- LLM
...
Serves [`Qwen/Qwen3-32B`](https://recipes.vllm.ai/Qwen/Qwen3-32B) from an
...
[vLLM recipe for Qwen3-32B](https://recipes.vllm.ai/Qwen/Qwen3-32B): every vLLM
...
Qwen3 32B dense model with hybrid thinking/non-thinking modes — verified on TPU
v6e (Trillium).
...
| --- | --- | --- | --- |
| `awq` | int4 | 20 GB | `Qwen/Qwen3-32B-AWQ` |
| `default` | bf16 | 77 GB | `Qwen/Qwen3-32B` |
| `fp8` | fp8 | 39 GB | `Qwen/Qwen3-32B-FP8` |

## Features

-  **`EnableToolCalling`** (default on): Enable automatic tool choice with the
  Hermes parser
-  **`EnableReasoning`** (default on): Enable Qwen3 thinking mode
  (chain-of-thought) with the Qwen3 reasoning parser

## Usage

```sh
fuzzball workflow catalog start vllm_qwen3_32b
fuzzball workflow catalog start vllm_qwen3_32b --values Hardware=mi355x,Variant=fp8
fuzzball workflow catalog start vllm_qwen3_32b --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `Qwen/Qwen3-32B` regardless of the variant served,
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/Qwen/Qwen3-32B. Regenerate this application with
