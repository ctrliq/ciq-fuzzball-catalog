...
---
id: "ciq/ml_and_ai/vllm_lfm2_5_1_2b_thinking"
name: "vLLM LFM2.5-1.2B-Thinking"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves
[`LiquidAI/LFM2.5-1.2B-Thinking`](https://recipes.vllm.ai/LiquidAI/LFM2.5-1.2B-Thinking)
...
from the [vLLM recipe for LFM2.5 1.2B
Thinking](https://recipes.vllm.ai/LiquidAI/LFM2.5-1.2B-Thinking): every vLLM
...
Liquid AI's 1.2B reasoning model on the LFM2 hybrid conv+attention backbone —
<think> chain-of-thought, tool calling, and 32K context on a single small GPU.

## Model

- **Checkpoint**: `LiquidAI/LFM2.5-1.2B-Thinking`
...
```sh
fuzzball workflow catalog start vllm_lfm2_5_1_2b_thinking
fuzzball workflow catalog start vllm_lfm2_5_1_2b_thinking --values Hardware=mi355x
fuzzball workflow catalog start vllm_lfm2_5_1_2b_thinking --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `LiquidAI/LFM2.5-1.2B-Thinking` regardless of the
...
vllm serve LiquidAI/LFM2.5-1.2B-Thinking \
  ...
  --served-model-name LiquidAI/LFM2.5-1.2B-Thinking
```
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/LiquidAI/LFM2.5-1.2B-Thinking. Regenerate this
