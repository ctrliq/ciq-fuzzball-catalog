...
---
id: "ciq/ml_and_ai/vllm_laguna_m_1"
name: "vLLM Laguna-M.1"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves [`poolside/Laguna-M.1`](https://recipes.vllm.ai/poolside/Laguna-M.1)
...
M.1](https://recipes.vllm.ai/poolside/Laguna-M.1): every vLLM flag, environment
variable, container image, GPU count and parallel layout below is what that
recipe validated for the selected hardware.

Poolside's 225B total / 23B activated MoE coding model with global attention,
native interleaved reasoning, and 256K context — designed for agentic coding
and long-horizon work.

## Model

- **Checkpoint**: `poolside/Laguna-M.1`
- **Architecture**: moe, 225B parameters
...
| --- | --- | --- | --- |
| `default` | bf16 | 540 GB | `poolside/Laguna-M.1` |
| `fp8` | fp8 | 270 GB | `poolside/Laguna-M.1-FP8` |
| `nvfp4` | nvfp4 | 135 GB | `poolside/Laguna-M.1-NVFP4` |
...
-  **`EnableReasoning`** (default on): Enable interleaved thinking (on by
  default) with Poolside's reasoning parser

## Usage

```sh
fuzzball workflow catalog start vllm_laguna_m_1
fuzzball workflow catalog start vllm_laguna_m_1 --values Hardware=mi355x,Strategy=single_node_tp,Variant=nvfp4
fuzzball workflow catalog start vllm_laguna_m_1 --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `poolside/Laguna-M.1` regardless of the variant
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/poolside/Laguna-M.1. Regenerate this application with
