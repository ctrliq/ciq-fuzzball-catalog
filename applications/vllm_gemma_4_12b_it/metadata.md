...
---
id: "ciq/ml_and_ai/vllm_gemma_4_12b_it"
...
Serves [`google/gemma-4-12B-it`](https://recipes.vllm.ai/Google/gemma-4-12B-it)
...
from the [vLLM recipe for Gemma 4 12B
IT](https://recipes.vllm.ai/Google/gemma-4-12B-it): every vLLM flag,
...
Google's encoder-free unified Gemma 4 dense model (12B) with native text,
...
| --- | --- | --- | --- |
| `default` | bf16 | 29 GB | `google/gemma-4-12B-it` |
| `w4a16` | int4 | 9 GB | `google/gemma-4-12B-it-qat-w4a16-ct` |
...
```sh
fuzzball workflow catalog start vllm_gemma_4_12b_it
fuzzball workflow catalog start vllm_gemma_4_12b_it --values Hardware=mi355x,Variant=w4a16
fuzzball workflow catalog start vllm_gemma_4_12b_it --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `google/gemma-4-12B-it` regardless of the variant
...
Image: `docker://vllm/vllm-openai:gemma4-unified`
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/Google/gemma-4-12B-it. Regenerate this application
