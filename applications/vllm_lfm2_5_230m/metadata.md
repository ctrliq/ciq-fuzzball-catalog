...
---
id: "ciq/ml_and_ai/vllm_lfm2_5_230m"
name: "vLLM LFM2.5-230M"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves [`LiquidAI/LFM2.5-230M`](https://recipes.vllm.ai/LiquidAI/LFM2.5-230M)
...
from the [vLLM recipe for LFM2.5
230M](https://recipes.vllm.ai/LiquidAI/LFM2.5-230M): every vLLM flag,
...
Liquid AI's most compact LFM2.5 chat model (230M) on the LFM2 hybrid
conv+attention backbone — distilled from LFM2.5-350M, tool calling and 32K
context, built for edge / on-device serving.

## Model

- **Checkpoint**: `LiquidAI/LFM2.5-230M`
- **Architecture**: dense, 230M parameters
- **Active parameters**: 230M
...
```sh
fuzzball workflow catalog start vllm_lfm2_5_230m
fuzzball workflow catalog start vllm_lfm2_5_230m --values Hardware=mi355x
fuzzball workflow catalog start vllm_lfm2_5_230m --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `LiquidAI/LFM2.5-230M` regardless of the variant
...
vllm serve LiquidAI/LFM2.5-230M \
  ...
  --served-model-name LiquidAI/LFM2.5-230M
```
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/LiquidAI/LFM2.5-230M. Regenerate this application
