...
Serves [`tencent/Hy3`](https://recipes.vllm.ai/tencent/Hy3) from an autoscaled
pool of vLLM ([vLLM docs](https://docs.vllm.ai/en/stable)) replicas behind a
single OpenAI-compatible base URL. Deployment parameters come from the [vLLM
recipe for Hy3](https://recipes.vllm.ai/tencent/Hy3): every vLLM flag,
...
Tencent Hy3 — scaled-up MoE language model (295B total / 21B active) with a
3.8B MTP layer for speculative decoding, 256K context, and hy_v3 tool/reasoning
parsers

## Model

- **Checkpoint**: `tencent/Hy3`
...
| `gb300` | 4x GB300 NVL4, 1152 GB | 1/4 | verified |
...
| --- | --- | --- | --- |
| `default` | bf16 | 708 GB | `tencent/Hy3` |
| `fp8` | fp8 | 354 GB | `tencent/Hy3-FP8` |
| `nvfp4` | nvfp4 | 214 GB | `RedHatAI/Hy3-NVFP4-FP8` |
...
-  **`EnableToolCalling`** (default on): Hy v3 tool call parser with automatic
  tool choice
-  **`EnableReasoning`** (default on): Hy v3 reasoning parser for thinking-mode
...
```sh
fuzzball workflow catalog start vllm_hy3
fuzzball workflow catalog start vllm_hy3 --values Hardware=mi355x,Strategy=single_node_tp,Variant=nvfp4
fuzzball workflow catalog start vllm_hy3 --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `tencent/Hy3` regardless of the variant served,
...
Image: `docker://vllm/vllm-openai:hy3`
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/tencent/Hy3. Regenerate this application with
