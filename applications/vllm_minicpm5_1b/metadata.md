...
---
id: "ciq/ml_and_ai/vllm_minicpm5_1b"
name: "vLLM MiniCPM5-1B"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves [`openbmb/MiniCPM5-1B`](https://recipes.vllm.ai/openbmb/MiniCPM5-1B)
...
MiniCPM5-1B](https://recipes.vllm.ai/openbmb/MiniCPM5-1B): every vLLM flag,
...
MiniCPM5-1B — dense 1B on-device LLM with hybrid Think/No-Think reasoning,
native 128K context, and strong agentic tool use, built on the standard Llama
architecture

## Model

- **Checkpoint**: `openbmb/MiniCPM5-1B`
- **Architecture**: dense, 1.1B parameters
- **Active parameters**: 1.1B
...
-  **`EnableReasoning`** (default off): Enable hybrid deep-thinking mode — the
  chat template wraps a <think> block before the answer. Recommended sampling
  shifts to temperature=0.9, top_p=0.95. Leave off for fast No-Think responses
  (temperature=0.7).

## Usage

```sh
fuzzball workflow catalog start vllm_minicpm5_1b
fuzzball workflow catalog start vllm_minicpm5_1b --values Hardware=mi355x
fuzzball workflow catalog start vllm_minicpm5_1b --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `openbmb/MiniCPM5-1B` regardless of the variant
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/openbmb/MiniCPM5-1B. Regenerate this application with
