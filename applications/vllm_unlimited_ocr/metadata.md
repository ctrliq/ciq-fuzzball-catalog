...
---
id: "ciq/ml_and_ai/vllm_unlimited_ocr"
name: "vLLM Unlimited-OCR"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves [`baidu/Unlimited-OCR`](https://recipes.vllm.ai/baidu/Unlimited-OCR)
...
Unlimited-OCR](https://recipes.vllm.ai/baidu/Unlimited-OCR): every vLLM flag,
...
Baidu's state-of-the-art document-parsing model with Reference Sliding Window
Attention (R-SWA), optimized for full-page OCR and markdown generation.

## Model

- **Checkpoint**: `baidu/Unlimited-OCR`
- **Architecture**: dense, 3B parameters
...
-  **`EnableEncoderParallel`** (default off): Run the vision encoder in
  data-parallel mode — avoids TP comm overhead on the small encoder.

## Usage

```sh
fuzzball workflow catalog start vllm_unlimited_ocr
fuzzball workflow catalog start vllm_unlimited_ocr --values Hardware=mi355x
fuzzball workflow catalog start vllm_unlimited_ocr --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `baidu/Unlimited-OCR` regardless of the variant
...
Image: `docker://vllm/vllm-openai:unlimited-ocr`
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/baidu/Unlimited-OCR. Regenerate this application with
