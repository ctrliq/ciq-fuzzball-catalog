...
---
id: "ciq/ml_and_ai/vllm_lfm2_5_vl_1_6b"
name: "vLLM LFM2.5-VL-1.6B"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves
[`LiquidAI/LFM2.5-VL-1.6B`](https://recipes.vllm.ai/LiquidAI/LFM2.5-VL-1.6B)
...
1.6B](https://recipes.vllm.ai/LiquidAI/LFM2.5-VL-1.6B): every vLLM flag,
...
Liquid AI's 1.6B vision-language model — LFM2 hybrid LM backbone plus a SigLIP2
vision tower for image+text chat on a single small GPU.

## Model

- **Checkpoint**: `LiquidAI/LFM2.5-VL-1.6B`
- **Architecture**: dense, 1.6B parameters
- **Active parameters**: 1.6B
...
```sh
fuzzball workflow catalog start vllm_lfm2_5_vl_1_6b
fuzzball workflow catalog start vllm_lfm2_5_vl_1_6b --values Hardware=mi355x
fuzzball workflow catalog start vllm_lfm2_5_vl_1_6b --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `LiquidAI/LFM2.5-VL-1.6B` regardless of the
...
vllm serve LiquidAI/LFM2.5-VL-1.6B \
  ...
  --served-model-name LiquidAI/LFM2.5-VL-1.6B
```
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/LiquidAI/LFM2.5-VL-1.6B. Regenerate this application
