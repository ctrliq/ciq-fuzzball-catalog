...
---
id: "ciq/ml_and_ai/vllm_step_3_7_flash"
name: "vLLM Step-3.7-Flash"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves
[`stepfun-ai/Step-3.7-Flash`](https://recipes.vllm.ai/stepfun-ai/Step-3.7-Flash)
...
Step-3.7-Flash](https://recipes.vllm.ai/stepfun-ai/Step-3.7-Flash): every vLLM
...
Production-grade vision-language MoE (~198B total / 11B active parameters)
combining a 196B sparse language backbone with a 1.8B perception encoder,
hybrid SWA/Global attention, and 3-way Multi-Token Prediction

## Model

- **Checkpoint**: `stepfun-ai/Step-3.7-Flash`
- **Architecture**: moe, 198B parameters
...
| --- | --- | --- | --- |
| `b200` | 8x B200, 1440 GB | 4/8 | verified |
...
| --- | --- | --- | --- |
| `default` | bf16 | 475 GB | `stepfun-ai/Step-3.7-Flash` |
| `fp8` | fp8 | 238 GB | `stepfun-ai/Step-3.7-Flash-FP8` |
| `nvfp4` | nvfp4 | 119 GB | `stepfun-ai/Step-3.7-Flash-NVFP4` |
...
-  **`EnableSpecDecoding`** (default off): 3-layer Multi-Token Prediction
  speculative decoding (MTP-3)

## Usage

```sh
fuzzball workflow catalog start vllm_step_3_7_flash
fuzzball workflow catalog start vllm_step_3_7_flash --values Hardware=mi355x,Strategy=single_node_tp,Variant=nvfp4
fuzzball workflow catalog start vllm_step_3_7_flash --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `stepfun-ai/Step-3.7-Flash` regardless of the
...
vllm serve stepfun-ai/Step-3.7-Flash \
  ...
  --served-model-name stepfun-ai/Step-3.7-Flash
```

Image: `docker://vllm/vllm-openai:stepfun37`
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/stepfun-ai/Step-3.7-Flash. Regenerate this
