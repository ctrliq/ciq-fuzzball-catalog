...
Serves [`zai-org/GLM-OCR`](https://recipes.vllm.ai/zai-org/GLM-OCR) from an
...
[vLLM recipe for GLM-OCR](https://recipes.vllm.ai/zai-org/GLM-OCR): every vLLM
...
GLM-OCR image-to-text model with built-in MTP speculative decoding for
high-throughput OCR serving
...
-  **`EnableSpecDecoding`** (default on): Multi-Token Prediction speculative
...
```sh
fuzzball workflow catalog start vllm_glm_ocr
fuzzball workflow catalog start vllm_glm_ocr --values Hardware=mi355x
fuzzball workflow catalog start vllm_glm_ocr --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `zai-org/GLM-OCR` regardless of the variant
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/zai-org/GLM-OCR. Regenerate this application with
