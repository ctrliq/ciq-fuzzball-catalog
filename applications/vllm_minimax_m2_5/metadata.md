...
---
id: "ciq/ml_and_ai/vllm_minimax_m2_5"
...
Serves
[`MiniMaxAI/MiniMax-M2.5`](https://recipes.vllm.ai/MiniMaxAI/MiniMax-M2.5) from
...
MiniMax-M2.5](https://recipes.vllm.ai/MiniMaxAI/MiniMax-M2.5): every vLLM flag,
...
MiniMax M2.5 MoE language model (230B total / 10B active) for coding, agent
toolchains, and long-context reasoning — native FP8 checkpoint

## Model

- **Checkpoint**: `MiniMaxAI/MiniMax-M2.5`
...
| --- | --- | --- | --- |
| `default` | fp8 | 276 GB | `MiniMaxAI/MiniMax-M2.5` |
| `nvfp4` | nvfp4 | 138 GB | `nvidia/MiniMax-M2.5-NVFP4` |
...
```sh
fuzzball workflow catalog start vllm_minimax_m2_5
fuzzball workflow catalog start vllm_minimax_m2_5 --values Hardware=mi355x,Strategy=single_node_tp,Variant=nvfp4
fuzzball workflow catalog start vllm_minimax_m2_5 --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `MiniMaxAI/MiniMax-M2.5` regardless of the variant
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/MiniMaxAI/MiniMax-M2.5. Regenerate this application
