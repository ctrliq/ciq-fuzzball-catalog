...
---
id: "ciq/ml_and_ai/vllm_lfm2_5_1_2b_jp"
name: "vLLM LFM2.5-1.2B-JP"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves
[`LiquidAI/LFM2.5-1.2B-JP`](https://recipes.vllm.ai/LiquidAI/LFM2.5-1.2B-JP)
...
JP](https://recipes.vllm.ai/LiquidAI/LFM2.5-1.2B-JP): every vLLM flag,
...
Liquid AI's 1.2B Japanese-specialized chat model on the LFM2 hybrid
conv+attention backbone, with a 32K context window on a single small GPU.

## Model

- **Checkpoint**: `LiquidAI/LFM2.5-1.2B-JP`
...
```sh
fuzzball workflow catalog start vllm_lfm2_5_1_2b_jp
fuzzball workflow catalog start vllm_lfm2_5_1_2b_jp --values Hardware=mi355x
fuzzball workflow catalog start vllm_lfm2_5_1_2b_jp --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `LiquidAI/LFM2.5-1.2B-JP` regardless of the
...
vllm serve LiquidAI/LFM2.5-1.2B-JP \
  ...
  --served-model-name LiquidAI/LFM2.5-1.2B-JP
```
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/LiquidAI/LFM2.5-1.2B-JP. Regenerate this application
