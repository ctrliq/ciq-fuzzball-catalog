...
---
id: "ciq/ml_and_ai/vllm_intern_s2_preview"
name: "vLLM Intern-S2-Preview"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves
[`internlm/Intern-S2-Preview`](https://recipes.vllm.ai/internlm/Intern-S2-Preview)
...
Intern-S2-Preview](https://recipes.vllm.ai/internlm/Intern-S2-Preview): every
...
Scientific multimodal MoE (36B total / 3B active) continued pre-trained from
Qwen3.5 — hybrid linear/full attention, 262K context, MTP-accelerated
reasoning. BF16 and FP8 checkpoints.

## Model

- **Checkpoint**: `internlm/Intern-S2-Preview`
- **Architecture**: moe, 36B parameters
...
| --- | --- | --- | --- |
| `default` | bf16 | 87 GB | `internlm/Intern-S2-Preview` |
| `fp8` | fp8 | 44 GB | `internlm/Intern-S2-Preview-FP8` |
...
-  **`EnableToolCalling`** (default on): Enable automatic tool choice with the
  Qwen3 Coder parser (per official deployment guide)
-  **`EnableReasoning`** (default on): Extract <think>...</think>
  chain-of-thought via the Qwen3 reasoning parser
-  **`EnableSpecDecoding`** (default off): Shared-weight MTP speculative
  decoding — 4 draft tokens (recommended in the deployment guide)
...
```sh
fuzzball workflow catalog start vllm_intern_s2_preview
fuzzball workflow catalog start vllm_intern_s2_preview --values Hardware=mi355x,Strategy=single_node_tp,Variant=fp8
fuzzball workflow catalog start vllm_intern_s2_preview --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `internlm/Intern-S2-Preview` regardless of the
...
vllm serve internlm/Intern-S2-Preview \
  ...
  --served-model-name internlm/Intern-S2-Preview
```
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/internlm/Intern-S2-Preview. Regenerate this
