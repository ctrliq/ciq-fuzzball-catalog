...
Serves [`zai-org/Glyph`](https://recipes.vllm.ai/zai-org/Glyph) from an
...
[vLLM recipe for Glyph](https://recipes.vllm.ai/zai-org/Glyph): every vLLM
...
Visual-text compression framework that renders long text into images and
processes them with a reasoning VLM, scaling effective context length

## Model

- **Checkpoint**: `zai-org/Glyph`
...
-  **`EnableReasoning`** (default on): GLM-4.5 reasoning parser for extracting
  reasoning traces from Glyph outputs
...
```sh
fuzzball workflow catalog start vllm_glyph
fuzzball workflow catalog start vllm_glyph --values Hardware=mi355x
fuzzball workflow catalog start vllm_glyph --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `zai-org/Glyph` regardless of the variant served,
...
vllm serve zai-org/Glyph \
  ...
  --served-model-name zai-org/Glyph
```
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/zai-org/Glyph. Regenerate this application with
