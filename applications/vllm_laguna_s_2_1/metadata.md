...
---
id: "ciq/ml_and_ai/vllm_laguna_s_2_1"
...
Serves [`poolside/Laguna-S-2.1`](https://recipes.vllm.ai/poolside/Laguna-S-2.1)
...
S-2.1](https://recipes.vllm.ai/poolside/Laguna-S-2.1): every vLLM flag,
...
Poolside's 118B total / 8B activated MoE coding model with mixed sliding-window
+ global attention, native interleaved reasoning, and 256K context — the larger
sibling of Laguna XS-2.1, tuned for agentic coding.

## Model

- **Checkpoint**: `poolside/Laguna-S-2.1`
- **Architecture**: moe, 118B parameters
...
| --- | --- | --- | --- |
| `default` | bf16 | 282 GB | `poolside/Laguna-S-2.1` |
| `fp8` | fp8 | 145 GB | `poolside/Laguna-S-2.1-FP8` |
| `int4` | int4 | 86 GB | `poolside/Laguna-S-2.1-INT4` |
| `nvfp4` | nvfp4 | 86 GB | `poolside/Laguna-S-2.1-NVFP4` |
...
-  **`EnableSpecDecoding`** (default off): DFlash speculative decoding with the
  Laguna-S-2.1 draft model (15 tokens, greedy)

## Usage

```sh
fuzzball workflow catalog start vllm_laguna_s_2_1
fuzzball workflow catalog start vllm_laguna_s_2_1 --values Hardware=mi355x,Strategy=single_node_tp,Variant=nvfp4
fuzzball workflow catalog start vllm_laguna_s_2_1 --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `poolside/Laguna-S-2.1` regardless of the variant
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/poolside/Laguna-S-2.1. Regenerate this application
