...
---
id: "ciq/ml_and_ai/vllm_mistral_medium_3_5_128b"
name: "vLLM Mistral-Medium-3.5-128B"
category: "ML_AND_AI"
tags:
- LLM
- inference
...
Serves
[`mistralai/Mistral-Medium-3.5-128B`](https://recipes.vllm.ai/mistralai/Mistral-Medium-3.5-128B)
...
Mistral-Medium-3.5](https://recipes.vllm.ai/mistralai/Mistral-Medium-3.5-128B):
...
Mistral Medium 3.5 (128B) dense vision-language model with native FP8 weights
and 256K context

## Model

- **Checkpoint**: `mistralai/Mistral-Medium-3.5-128B`
- **Architecture**: dense, 128B parameters
- **Active parameters**: 128B
- **Context length**: 262144 tokens
- **Minimum vLLM version**: 0.18.0
...
-  **`EnableReasoning`** (default on): Mistral reasoning parser extracts
  [THINK]...[/THINK] into message.reasoning (emitted when
...
-  **`EnableSpecDecoding`** (default off): EAGLE speculative decoding via
  mistralai/Mistral-Medium-3.5-128B-EAGLE draft head

## Usage

```sh
fuzzball workflow catalog start vllm_mistral_medium_3_5_128b
fuzzball workflow catalog start vllm_mistral_medium_3_5_128b --values Hardware=mi355x
fuzzball workflow catalog start vllm_mistral_medium_3_5_128b --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `mistralai/Mistral-Medium-3.5-128B` regardless of
...
Image: `docker://vllm/vllm-openai:v0.18.0`
...
-  The recipe lists extra install steps that apply to its pip install path; the
  ...
  -  `uv pip install -U "mistral_common>=1.11.1"`
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/mistralai/Mistral-Medium-3.5-128B. Regenerate this
