...
Serves [`openai/gpt-oss-120b`](https://recipes.vllm.ai/openai/gpt-oss-120b)
...
GPT-OSS](https://recipes.vllm.ai/openai/gpt-oss-120b): every vLLM flag,
...
OpenAI's gpt-oss family (20B / 120B) with MXFP4 MoE, attention-sinks, built-in
tools via Responses API
...
| --- | --- | --- | --- |
| `amd_fp8` | mxfp4 | 80 GB | `amd/gpt-oss-120b-w-mxfp4-a-fp8` |
| `default` | mxfp4 | 96 GB | `openai/gpt-oss-120b` |

## Features

-  **`EnableToolCalling`** (default on): OpenAI harmony tool-call parser with
  automatic tool choice
-  **`EnableSpecDecoding`** (default off): EAGLE3 speculative decoding for
  accelerated inference

## Usage

```sh
fuzzball workflow catalog start vllm_gpt_oss_120b
fuzzball workflow catalog start vllm_gpt_oss_120b --values Hardware=mi355x,Variant=default
fuzzball workflow catalog start vllm_gpt_oss_120b --values ModelVolume=volume://user/models,MaxReplicas=4
```

Clients address the model as `openai/gpt-oss-120b` regardless of the variant
...
-  Deployment parameters track the recipe at
  https://recipes.vllm.ai/openai/gpt-oss-120b. Regenerate this application with
